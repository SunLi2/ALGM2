...
class MLPBlock(nn.Module):
    def __init__(self, dim, n_div, mlp_ratio, drop_path, layer_scale_init_value, act_layer, norm_layer, pconv_fw_type):
        ...
        #     self.globalAvgPool = nn.AvgPool2d(spatial_size)
        ...
        self.fc1 = nn.Linear(in_features=dim, out_features=round(dim/4))
        self.fc2 = nn.Linear(in_features=round(dim/4), out_features=dim)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # 应用注意力权重
        x = shortcut + self.drop_path(att * self.mlp(x))
        return x
...
# FastNet(True)
#
# x=torch.randn(10,6,473,477).cuda()
# import time
#
# m=FasterNet().cuda()
