...
from networks.encoders.swin_fast import MatteFormer
# from networks.encoders.swin import MatteFormer
# from networks.encoders.fastnet import FasterNet
from networks.encoders.fast_se import FastNet
# from networks.encoders.fast_sa import FastNet
# from test import Create_Matteformer
# fg_encoder=Create_Matteformer()
# fg_encoder.cuda()
...
class Generator_MatteFormer(nn.Module):
    ...
    def __init__(self, is_train=True):
        ...
        self.encoder=FastNet(is_train)
        # self.encoder=FasterNet()
        ...
        # if is_train:
        #     self.init_pretrained_weight2(pretrained_path=CONFIG.model.imagenet_pretrain_path)  # MatteFormer
    ...
    def init_pretrained_weight2(self, pretrained_path=None):
        if not os.path.isfile(pretrained_path):
            print('Please Check your Pretrained weight path.. file not exist : {}'.format(pretrained_path))
            exit()
        ...
        for layer_name in attn_layers:
            ...
            layer_idx, block_idx = int(layer_name.split('.')[1]), int(layer_name.split('.')[3])
            n_prior = block_idx + 1
            pos_bias_new = torch.nn.init.xavier_normal_(torch.randn(n_bias, n_head))
        ...
        for layer_name in attn_layers:
            ...
            num_patch = 49
            last_idx = 169
            pos_index_new = torch.ones((num_patch, num_patch)).long() * last_idx
            pos_index_new[:num_patch, :num_patch] = pos_index
            for i in range(n_prior):
                for j in range(3):
                    pos_index_new[:, num_patch + j:num_patch +j +1] = last_idx + j
            weight_[layer_name] = pos_index_new

        self.encoder.load_state_dict(weight_, strict=False)
        print('load pretrained model done')

    def forward(self, image, trimap,alpha):
    # def forward(self, image, trimap):
        inp = torch.cat((image, trimap), axis=1)
        # x = self.encoder(inp,trimap)
        x = self.encoder(inp)
        embedding = x[-1]
        outs = self.decoder(embedding, x[:-1],image,alpha)
        # outs = self.decoder(embedding, x[:-1])
        return outs


    # def forward(self, image, trimap, fg_norm, istrain):
    #     inp = torch.cat((image, trimap), axis=1)
    #     x = self.encoder(inp, trimap)
    #     embedding = x[-1]
    #     outs = self.decoder(embedding, x[:-1])
    #     # if istrain:
    #     #     inp_lable = torch.cat((fg_norm, trimap), axis=1)
    #     #     x_lable = fg_encoder(inp_lable, trimap)
    #     #     return outs, x, x_lable
    #     # else:
    #     return outs
