    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, norm_layer=None, large_kernel=False):
        ...
        if self.stride > 1:
            self.conv1 = SpectralNorm(
                nn.ConvTranspose2d(inplanes, inplanes, kernel_size=4, stride=2, padding=1, bias=False))
        else:
...
class ResNet_D_Dec(nn.Module):
    ...
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False):
        ...
        self.conv1_1_1 = nn.Conv2d(in_channels=3, out_channels=256, kernel_size=16, stride=16, padding=0)
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=128, kernel_size=8, stride=8, padding=0)
        self.conv3 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=4, stride=4, padding=0)
        self.conv4 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=2, stride=2, padding=0)

        self.conv1_1 = nn.Conv2d(in_channels=512, out_channels=256, kernel_size=1)
        self.conv2_1 = nn.Conv2d(in_channels=256, out_channels=128, kernel_size=1)
        self.conv3_1 = nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1)
        self.conv4_1 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=1)
        ...
        self.refine_OS1 = nn.Sequential(
            ...
            norm_layer(32),
            self.leaky_relu,
            nn.Conv2d(32, 3, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size // 2), )
...
class ResShortCut_D_Decoder(ResNet_D_Dec):
    ...
    def __init__(self, block, layers, norm_layer=None, large_kernel=False, late_downsample=False):
        super(ResShortCut_D_Decoder, self).__init__(block, layers, norm_layer, large_kernel,
                                                    late_downsample=late_downsample)

    def forward(self, x, mid_fea,image,alpha):
        ret = {}
        fea1, fea2, fea3, fea4, fea5 = mid_fea

        alphaI=image*alpha
        alphaI1=self.conv1_1_1(alphaI)
        alphaI2=self.conv2(alphaI)
        alphaI3=self.conv3(alphaI)
        alphaI4=self.conv4(alphaI)

        # image_32 = F.interpolate(alphaI, size=(fea2.shape[2], fea2.shape[3]), mode='bilinear', align_corners=False)
        # image_64 = F.interpolate(alphaI, size=(fea3.shape[2], fea3.shape[3]), mode='bilinear', align_corners=False)
        # image_128 = F.interpolate(alphaI, size=(fea4.shape[2], fea4.shape[3]), mode='bilinear', align_corners=False)
        # image_256 = F.interpolate(alphaI, size=(fea5.shape[2], fea5.shape[3]), mode='bilinear', align_corners=False)

        x = self.layer1(x) + fea5
        short=x
        x=torch.cat((x,alphaI1),dim=1)
        x=self.conv1_1(x)
        x=x+short

        x = self.layer2(x) + fea4
        short = x
        x = torch.cat((x, alphaI2), dim=1)
        x = self.conv2_1(x)
        x = x + short
        x_os8 = self.refine_OS8(x)
...
# m=ResShortCut_D_Decoder(BasicBlock, [2, 3, 3, 2])
# tensor_list = []
# trimap=torch.randn(1,1,512,512)
# alpha=torch.randn(1,1,512,512)
# image=torch.randn(1,3,512,512)
# shapes = [(1, 32, 512, 512), (1, 32, 256, 256), (1, 64, 128, 128), (1, 128, 64, 64), (1, 256, 32, 32),(1,512,16,16)]
# x=torch.randn(1,512,16,16)
# for shape in shapes:
#     tensor = torch.randn(*shape)  # 生成随机数据，您可以替换为其他数据
#     tensor_list.append(tensor)
# x=m(x,tensor_list[:5],image,alpha)
