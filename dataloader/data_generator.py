...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # alpha_three = np.zeros((alpha.shape[0], alpha.shape[1], 3), dtype=np.uint8)
        #
        if self.phase!="train":

            alpha_f = sample['alpha_f']
            sample['alpha_f']=torch.from_numpy(np.array(sample['alpha_f'].astype(np.float32)))
            alpha_f = np.expand_dims(alpha_f.astype(np.float32), axis=0)
            alpha_f= torch.from_numpy(alpha_f)
            alpha_three=torch.cat((alpha_f, alpha_f, alpha_f), dim=0)
            sample['alpha_three']=alpha_three
        else:

            alpha_f = torch.from_numpy(alpha)
            alpha_three = torch.cat((alpha_f, alpha_f, alpha_f), dim=0)
            sample['alpha_three'] = alpha_three
        ...
        image /= 255.
        # fggt = sample['fggt']
        # fggt=fggt.transpose(2, 0, 1)
        # fggt /=255.

        # plt.imshow(sample['fggt'])
        # plt.show()

        fggt = sample['fggt'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32)/ 255.

        # fggt=fggt.transpose((1,2,0))*255.
        # plt.imshow(np.uint8(fggt))
        # plt.show()
        # bg[:, :, [0, 2]] = bg[:, :, [2, 0]]
        # import matplotlib.pyplot as plt
        # plt.imshow(np.uint8(bg))
        # plt.show()

        # fggt = sample['fggt'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32)
        if self.phase == "train":
            # convert GBR images to RGB
            fg = sample['fg'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32) / 255.
            sample['fg'] = torch.from_numpy(fg).sub_(self.mean).div_(self.std)
            sample['bg'] = sample['bg'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32) / 255.
            sample['bg'] = torch.from_numpy(sample['bg']).sub_(self.mean).div_(self.std)


            # sample['bg_random']=sample['bg_random'][:, :, ::-1].transpose((2, 0, 1)).astype(np.float32) / 255.
            # sample['bg_random'] = torch.from_numpy(bg).sub_(self.mean).div_(self.std)
        ...
        sample['image'], sample['alpha'], sample['trimap'], sample['fggt'] = \
            torch.from_numpy(image), torch.from_numpy(alpha), torch.from_numpy(trimap).to(torch.long), torch.from_numpy(fggt)
...
class RandomAffine(object):
    ...
    def __call__(self, sample):
        fg, alpha, fggt = sample['fg'], sample['alpha'], sample['fggt']
        ...
        fggt = cv2.warpAffine(fggt, M, (cols, rows),
                              flags=maybe_random_interp(cv2.INTER_NEAREST) + cv2.WARP_INVERSE_MAP)
        sample['fg'], sample['alpha'], sample['fggt'] = fg, alpha, fggt
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        fg, alpha, trimap, mask, name, fggt = sample['fg'], sample['alpha'], sample['trimap'], sample['mask'], sample[
            'image_name'], sample['fggt']
        bg = sample['bg']
        h, w = trimap.shape
        bg = cv2.resize(bg, (w, h), interpolation=maybe_random_interp(cv2.INTER_CUBIC))
        # bg = bg[bg.shape[0] // 2 - h // 2: bg.shape[0] // 2 + h // 2, bg.shape[1] // 2 - w // 2: bg.shape[1] // 2 + w // 2]
        # bg[:, :, [0, 2]]=bg[:,:, [2, 0]]
        # plt.imshow(bg)
        # plt.show()
        if w < self.output_size[0] + 1 or h < self.output_size[1] + 1:
            ...
            while h < self.output_size[0] + 1 or w < self.output_size[1] + 1:
                fggt = cv2.resize(fggt, (int(w * ratio), int(h * ratio)),
        ...
        fggt_crop = fggt[left_top[0]:left_top[0] + self.output_size[0], left_top[1]:left_top[1] + self.output_size[1],
                    :]
        ...
        if len(np.where(trimap == 128)[0]) == 0:
            ...
            fggt_crop = cv2.resize(fggt, self.output_size[::-1], interpolation=maybe_random_interp(cv2.INTER_NEAREST))
...
class OriginScale(object):
    def __call__(self, sample):
        ...
        padded_fggt = np.pad(sample['fggt'], ((0, pad_h), (0, pad_w), (0, 0)), mode="reflect")
        sample['image'] = padded_image
        sample['trimap'] = padded_trimap
        sample['mask'] = padded_mask
        sample['fggt'] = padded_fggt

        sample['alpha_f'] = padded_alpha
...
class DataGenerator(Dataset):
    ...
    def __getitem__(self, idx):
        if self.phase == "train":
            ...
            bg = cv2.imread(self.bg[idx%2000])
            # bg_random = cv2.imread(self.bg[(idx + 1) % self.bg_num], 1)
            ...
            sample = {'fg': fg, 'alpha': alpha, 'bg': bg, 'image_name': image_name, 'fggt': fggt}


        else:
            image = cv2.imread(self.merged[idx])
            fggt = cv2.imread(self.fggt[idx])
            ...
            sample = {'image': image, 'alpha': alpha, 'trimap': trimap, 'mask': mask, 'image_name': image_name,
                      'alpha_shape': alpha.shape, 'fggt': fggt,'alpha_f':alpha}
