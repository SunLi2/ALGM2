...
def single_inference(model, image_dict):
    ...
    with torch.no_grad():
        ...
        image_no = image_dict['imageno'].cuda()
        # image = image
        # trimap = trimap
        # image_no = image_dict['imageno']
        # alpha = image_dict['alpha']
        # alpha_three = image_dict['alpha_three']
        image_no=image_no.permute(2, 0, 1)

        # run model
        # pred = model(image, trimap,trimap_one)
        ...
        indices = np.where(trimap.cpu().numpy()[0] == 2)
        if len(indices[0]) > 0:
            alpha_pred[:, indices[0]] = image_no[:, indices[0]]
...
def generator_tensor_dict(image_path, trimap_path,alpha_path):
    ...
    trimap = cv2.imread(trimap_path.replace('jpg','png'), 0)
    alpha = cv2.imread(alpha_path.replace('jpg','png'),0)/255
    ...
    if h % 32 == 0 and w % 32 == 0:
        ...
        padded_image_no = np.pad(sample['imageno'], ((32, 32), (32, 32), (0, 0)), mode="reflect")

        sample['image'] = padded_image
        sample['trimap'] = padded_trimap
        sample['alpha'] = padded_alpha
        sample['imageno'] = padded_image_no
        ...
        padded_image_no = np.pad(sample['imageno'], ((32, pad_h + 32), (32, pad_w + 32), (0, 0)), mode="reflect")
    ...
    # to tensor
    sample['imageno']=torch.from_numpy(sample['imageno'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint', type=str,
                        # default="/home/ljh/SLL/data/sll/MattingCode/AAAA/matteformer_fg_alpha/experiments/240530_161338/checkpoints/best_model_mse.pth",
                        default= '/home/ljh/SLL/data/sll/MattingCode/AAAA/matteformer_fg_alpha/experiments/240531_111157/checkpoints/best_model_mse.pth',
    ...
    #                     default='/home/ljh/SLL/data/sll/MattingCode/DataSet/Transparent-460/Test/alpha_copy/',
    ...
    # parser.add_argument('--mask-dir', type=str,
    #                     default='/home/ljh/SLL/data/sll/MattingCode/AEMatter/460/',
    #                     help="input trimap dir")
    # parser.add_argument('--image-dir', type=str,
    ...
    # parser.add_argument('--image-dir', type=str,
    #                     default='/home/ljh/SLL/data/sll/MattingCode/DataSet/AIM-test/merged/',
    #                     help="input image dir")
    # parser.add_argument('--mask-dir', type=str,
    #                     default='/home/ljh/SLL/data/sll/MattingCode/DataSet/AIM-test/alpha/',
    #                     help="input trimap dir")
    # parser.add_argument('--trimap-dir', type=str,
    #                     default='/home/ljh/SLL/data/sll/MattingCode/DataSet/AIM-test/trimap/',
    #                     help="input trimap dir")
    ...
    parser.add_argument('--output', type=str, default='/home/ljh/SLL/data/sll/MattingCode/AAAA/matteformer_fg_alpha/preDIM/AEM/', help="output dir")
    ...
    utils.make_dir(os.path.join(args.output, '240531_111157_com'))
    ...
    for i, image_name in enumerate(os.listdir(args.image_dir)):
        ...
        trimap_path = os.path.join(args.trimap_dir, image_name.replace('png','jpg'))
        alpha_path = os.path.join(args.mask_dir, image_name.replace('png','jpg'))
        ...
        cv2.imwrite(os.path.join(args.output, '240531_111157_com', image_name), alpha_pred)
